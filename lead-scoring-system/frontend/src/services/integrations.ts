...
export type PipedriveConnectPayload = {
  credentials: {
    api_token: string;
    base_url?: string;
  };
...
export async function connectPipedriveIntegration(payload: PipedriveConnectPayload) {
  const { data } = await apiClient.post<CRMIntegration>("/crm/pipedrive/connect", payload);
  return data;
}
...
export async function fetchPipedriveStatus() {
  const { data } = await apiClient.get<CRMSyncStatusResponse>("/crm/pipedrive/status");
  return data;
}

export async function triggerPipedriveSync(payload: SyncTriggerPayload = {}) {
  const { data } = await apiClient.post<CRMSyncStatusResponse>("/crm/pipedrive/sync", payload);
  return data;
}
