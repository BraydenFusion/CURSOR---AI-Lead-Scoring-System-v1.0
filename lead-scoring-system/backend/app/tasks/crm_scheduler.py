    ...
    async with _session_scope() as session:
        if provider == "salesforce":
            if direction in ("to_crm", "bidirectional"):
                await sync_to_salesforce(integration_id, db=session)
            if direction in ("from_crm", "bidirectional"):
                await sync_from_salesforce(integration_id, db=session)
        elif provider == "hubspot":
            if direction in ("to_crm", "bidirectional"):
                await sync_to_hubspot(integration_id, db=session)
            if direction in ("from_crm", "bidirectional"):
                await sync_from_hubspot(integration_id, db=session)
        else:
