"""API routes for Pipedrive CRM integrations."""
...
def connect_pipedrive(
    ...
    integration = (
        db.query(CRMIntegration)
        .filter(CRMIntegration.user_id == current_user.id, CRMIntegration.provider == "pipedrive")
        .one_or_none()
    )
...
async def trigger_pipedrive_sync(
    payload: CRMSyncTriggerRequest,
    ...
    if not integration:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pipedrive integration not found.")
    ...
    async def _sync() -> None:
        async with _session_scope() as session:
            if direction in ("to_crm", "bidirectional"):
                await sync_to_pipedrive(integration.id, db=session)
            if direction in ("from_crm", "bidirectional"):
                await sync_from_pipedrive(integration.id, db=session)

    background_tasks.add_task(asyncio.create_task, _sync())
    return pipedrive_status(db=db, current_user=current_user)
